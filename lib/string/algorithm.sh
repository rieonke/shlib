...
function core::string::index_of_offset() {
    ...
    do
        # find first char
        local find_first=${i}
        while [[ ${find_first} -lt ${str_len} ]] && [[ ${str:${find_first}:1} != ${find:0:1} ]]
        do
            let "find_first ++ "
        done
        ...
        local j=0
        local k=${find_first}
        while [[ ${j} < ${find_len}  && ${str:${k}:1} == ${find:${j}:1}  ]]
        do
            let "j ++"
            let "k ++ "
        done
...
function core::string::split() {
    ...
    while [[ ${next_occurrence} -ge 0 && ${idx_offset} -lt $((${str_len})) ]]
    do
