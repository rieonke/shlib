...
function libsh_test_init() {
    local script="${1}"
    local sourced_arr=()
    for routine in $(core::__loader__::get_deps_routines ${script} . ${script} .. | sort -r | awk '{print $2}')
    do
        for el in $(echo ${routine} | awk '{ p = split($0, p_a, "#"); for (i=p;i>0;i--) { print p_a[i] } }')
        do
            if [[ ${el} == "." ]]
            then
                continue
...
#            fi
            ...
            if [[ $(__array_contains ${el} "${sourced_arr[@]}" )  -ge 0 ]]
            then
                    continue
                fi
